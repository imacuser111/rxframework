...
public extension ThemeProxy where Base: UISearchBar {
    ...
    /// (set only) bind a stream to keyboardAppearance
    var text: ThemeAttribute<String?> {
        get { fatalError("set only") }
        set {
            base.text = newValue.value ?? ""
            let disposable = newValue.stream
                .take(until: base.rx.deallocating)
                .observe(on: MainScheduler.instance)
                .bind(to: base.rx.text)
            hold(disposable, for: "text")
        }
    }

    /// (set only) bind a stream to keyboardAppearance
    var placeHolder: ThemeAttribute<String?> {
        get { fatalError("set only") }
        set {
            base.placeholder = newValue.value ?? ""
            let disposable = newValue.stream
                .take(until: base.rx.deallocating)
                .observe(on: MainScheduler.instance)
                .bind(to: base.rx.placeholder)
            hold(disposable, for: "placeholder")
        }
    }
